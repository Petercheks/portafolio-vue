...
<template>
  <div class="relative overflow-hidden max-w-4xl mx-auto">
    <div class="flex transition-transform duration-500 ease-in-out"
      :style="`transform: translateX(-${currentIndex * 100}px)`" @mouseenter="stop" @mouseleave="start">
      <div v-for="(skill, index) in [...skills, ...skills, ...skills, ...skills]" :key="`${index}-${skill.label}`"
